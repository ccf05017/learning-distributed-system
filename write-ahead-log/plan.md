...
- [x] 빈 키 거부 (PUT, DELETE)
- [x] 긴 키 정상 동작 (10,000자)
- [x] 큰 값 정상 동작 (100,000자)
- [x] 빈 값 허용

### ~~11.2 디스크 쓰기 실패 (G2)~~
> **제외**: 실제 디스크 쓰기는 sync() 시점에 발생하며, 이는 Phase 7의 TestSyncFailureRollback에서 이미 다룸
> - sync 실패 시 WAL 롤백으로 불완전 레코드 없음
> - sync 실패한 연산은 재시작 후에도 복구 안 됨 (결정적)

### 11.3 동시성 (G3)
- [x] 멀티 스레드 동시 쓰기 테스트 (10 스레드 × 100 쓰기)
- [x] KVStore 레벨 Lock으로 원자성 보장
- [x] WAL 체크섬 hex 인코딩으로 개행문자 충돌 해결

> **학습 포인트**: 동시성 테스트 중 발견된 버그가 실제로는 직렬화 포맷 문제였음.
> 바이너리 체크섬에 `\n`(0x0a)이 포함될 수 있어 줄 기반 파싱이 깨짐.
> 자세한 분석은 `docs/debugging-lesson-checksum-newline.md` 참조.
...
- [x] Phase 11 완료 - 경계/예외 케이스 (G1 경계값, G3 동시성)
- [ ] Phase 12 진행 예정 - 고급 엣지케이스
